...
import { type ZodType, z, type ZodError } from "zod";
...
export const createDefaultError = <T>(key: T) => ({
  required_error: `${key} cannot be blank` as const,
  invalid_type_error: `Invalid type provided for ${key}` as const,
});
...
export const handleOneLevelZodError = ({ issues }: ZodError<unknown>) => {
  ...
  const ele = (issues[0]?.path.length as any) < 1;
  //?.[0]?.path?.length < 1

  /** line of code should be true if the schema is not an object */
  if (issues.length === 1 && ele) return issues[0]?.message;
